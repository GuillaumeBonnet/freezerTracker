...
@Component({
	template: ` <app-freezerContent> </app-freezerContent>`,
})
...
describe('FreezerContent', () => {
	...
	let fakeFreezerContent: Aliment[] = [
		new Aliment({
			...
			quantity: 2,
			quantityUnit: 'kg',
			iconicFontName: 'icone2',
			expirationDate: new Date('2020-07-08'),
		...
		new Aliment({
			...
			quantity: 3,
			quantityUnit: 'kg',
			iconicFontName: 'icone3',
			expirationDate: new Date('2020-08-08'),
			storedDate: new Date('2019-08-08'),
		}),
	]
	...
	it('should requests and display Aliments', async () => {
		setTestHost('default')
		expect(dataServiceSpy.getFreezerContent).toHaveBeenCalledWith(
			freezerIdUrlParam
		)
		fixture.detectChanges()
		let alimentCards: NodeListOf<HTMLElement> =
			component.querySelectorAll('app-aliment')
	...
	it('should click a selected aliment, change its appearance and feed the aliment detail component ', () => {
		...
		expect(
			component.querySelector('app-aliment-details').textContent
		).toContain('Name :  name2')
		expect(
			component.querySelector('app-aliment-details').textContent
		).not.toContain('Name :  name3')
		//state after clicking unselected aliment:
		;(<HTMLElement>(
			component.querySelectorAll('app-aliment')[1].children[0]
		)).click()
		fixture.detectChanges()
		expect(
			component.querySelector('app-aliment-details').textContent
		).toContain('Name :  name3')
		expect(
			component.querySelector('app-aliment-details').textContent
		).not.toContain('Name :  name2')
	})
