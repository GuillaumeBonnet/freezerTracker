...
@Component({
	template: ` <app-pop-up-delete> </app-pop-up-delete>`,
})
...
describe('PopUpDeleteComponent', () => {
	describe('with testHostComponent', () => {
		...
		beforeEach(async(() => {
			TestBed.configureTestingModule({
				declarations: [PopUpDeleteComponent, TestHostComponent],
				providers: [
		...
		it('should display the right qestion', () => {
			expect(
				component.query(By.css('div.gs-text')).nativeElement.textContent
		...
		it('should close pop-up when cancel button is clicked', () => {
			let cancelButton: HTMLElement = component.query(
				By.css('button#close-button')
			).nativeElement
			cancelButton.click()
			// expect(dialogMockMenu.close).toHaveBeenCalled();
		})
		it('should close pop-up when cross is clicked', () => {
			let crossButton: HTMLElement = component.query(
				By.css('button#cross-button')
			).nativeElement
			crossButton.click()
			// expect(freezerMenuMatDialogSpy.close).toHaveBeenCalled();
		})
		it('should delete the right object', () => {
			let deleteButton: HTMLElement = component.query(
				By.css('button#delete-button')
			).nativeElement
			deleteButton.click()
			expect(dataServiceSpy.deleteAliment).toHaveBeenCalledWith(
				matDialogData.freezerId,
				new Aliment({ id: matDialogData.alimentId })
			)
			// expect(dialogMockMenu.close).toHaveBeenCalled();
			...
			fixture.detectChanges()
			deleteButton.click()
			expect(dataServiceSpy.deleteFreezer).toHaveBeenCalledWith(
				matDialogData.freezerId
			)
			// expect(dialogMockMenu.close).toHaveBeenCalled();
		})
	...
	describe('with DialogTestModule', () => {
		...
		it('should open and create the popup', () => {
			dialog.open(PopUpDeleteComponent, {
				width: '250px',
				panelClass: 'gs-popup',
				data: {
					freezerId: 1,
			...
			let component =
				overlayContainerElement.querySelector('app-pop-up-delete')
			expect(component).toBeTruthy()
		})
