...
export class PopUpDeleteComponent implements OnInit {
	constructor(
		...
		public dialogRef: MatDialogRef<PopUpFreezerMenuComponent>,
		@Inject(MAT_DIALOG_DATA)
		public data: { freezerId: number; alimentId: number }
	) {}
	...
	delete() {
		...
		if (this.data.alimentId) {
			this.dataService
				.deleteAliment(
					this.data.freezerId,
					new Aliment({ id: this.data.alimentId })
				)
				.subscribe(deleteResourceObsertver)
		} else {
			this.dataService
				.deleteFreezer(this.data.freezerId)
				.subscribe(deleteResourceObsertver)
		}
	}
