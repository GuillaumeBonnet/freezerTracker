...
@Component({
	template: ` <app-new-aliment [isEdit]="isEdit"> </app-new-aliment>`,
})
...
describe('NewAlimentComponent', () => {
	...
	dataServiceSpy.getFreezerContent.and.returnValue(
		of([alimentToFeedFormWith, secondAlim])
	)
	let component: HTMLElement
	let stubedRoute = {
		params: of({
			freezerId: freezerIdUrlParam,
			alimentId: alimentIdUrlParam,
		}),
	}

	beforeEach(async(() => {
		TestBed.configureTestingModule({
			declarations: [NewAlimentComponent, TestHostComponent],
			providers: [
				{ provide: MatDialog },
	...
	it('should close the form when the cancel button is clicked', () => {
		let cancelButton: HTMLElement = component.querySelector(
	...
	it('should highlight the next formfield when the left arrow is clicked', () => {
		let nextFieldButton: HTMLElement = component.querySelector(
			'button#next-field-button'
		)
		let formFields: NodeListOf<HTMLElement> =
			component.querySelectorAll('[id^=form-field]')
		expect(formFields.length).toBe(6)
		expect(formFields.item(0).classList).toContain(
			'mainbox__aliment--selected'
		)
		expect(formFields.item(1).classList).not.toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(2).classList).not.toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(3).classList).not.toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(4).classList).not.toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(5).classList).not.toContain(
			'mainbox__line--selected'
		)

		nextFieldButton.click()
		fixture.detectChanges()
		expect(formFields.item(0).classList).not.toContain(
			'mainbox__aliment--selected'
		)
		expect(formFields.item(1).classList).toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(2).classList).not.toContain(
			'mainbox__line--selected'
		...
		expect(formFields.item(2).classList).toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(3).classList).not.toContain(
			'mainbox__line--selected'
		...
		expect(formFields.item(3).classList).toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(4).classList).not.toContain(
			'mainbox__line--selected'
		...
		expect(formFields.item(4).classList).toContain(
			'mainbox__line--selected'
		)
		expect(formFields.item(5).classList).not.toContain(
			'mainbox__line--selected'
		...
		expect(formFields.item(5).classList).toContain(
			'mainbox__line--selected'
		)
	...
	it('should highlight the previous formfield when the left arrow is clicked', () => {
		let previousFieldButton: HTMLElement = component.querySelector(
	...
	it('should prevent form validation and dirty the field when the form is validated too soon', () => {
		let submitButton: HTMLElement = component.querySelector(
	...
	it('should create a new aliment when the form is correctly filled and navigate to the page ', () => {
		let appNewAliment = fixture.debugElement.query(
			By.css('app-new-aliment')
		).componentInstance
		...
		formControls.expirationDate.setValue(
			alimentToFeedFormWith.expirationDate
		)
		...
		expect(dataServiceSpy.addAliment).toHaveBeenCalledWith(
			freezerIdUrlParam,
			alimentToFeedFormWith
		)
	...
	it('should create comp in edit mode', () => {
		...
		alimentToFeedFormWith.id = alimentIdUrlParam
		expect(dataServiceSpy.editAliment).toHaveBeenCalledWith(
			freezerIdUrlParam,
			alimentToFeedFormWith
		)
		// expect(routerSpy.navigate).toHaveBeenCalledWith(['freezers', freezerIdUrlParam]);
