...
export class NewAlimentComponent implements OnInit {
	...
	currentInputPage: { value: string; index: number } = {
		value: INPUT_PAGES[0],
		index: 0,
	}
	...
	ngOnInit() {
		...
		this.alimentForm = this.fb.group({
			...
			quantityValue: this.fb.control('', [
				Validators.required,
				Validators.maxLength(250),
				this.validateNumber,
			]),
			quantityUnit: this.fb.control('', [
				Validators.required,
				Validators.maxLength(250),
			]),
			id: this.fb.control('', [
		...
		if (this.isEdit) {
			this.dataService.getFreezerContent(this.freezerId).subscribe({
				next: (freezerContent: Aliment[]) => {
					let alimToEdit = freezerContent.find(
						(alim) => alim.id == this.alimentId
					)
					if (alimToEdit) {
				...
				error: (error) => {
					console.log(
						'Could not find the freezer content error:',
						error
					)
					this.router.navigate(['freezers', this.freezerId])
				},
	...
	previousFromControl() {
		...
		this.currentInputPage.index =
			(this.currentInputPage.index - 1 + tabSize) % tabSize
	...
	nextFormControl() {
		let tabSize = INPUT_PAGES.length
		this.currentInputPage.index =
			(this.currentInputPage.index + 1) % tabSize
		this.currentInputPage.value = INPUT_PAGES[this.currentInputPage.index]
	}
	...
	setCurrentFormControl(value: string) {
		if (!INPUT_PAGES.includes(value)) {
			...
			this.currentInputPage.index = INPUT_PAGES.findIndex(
				(elem) => elem == value
			)
		}
	}
	...
	register() {
		if (this.alimentForm.valid) {
			...
			let dmlObserver: PartialObserver<any> = {
				...
				error: (error) => {
					console.log(
						'error with dml operation insert or update on an aliment:',
						error
					)
					this.router.navigate(['freezers', this.freezerId])
				},
			...
			if (this.isEdit) {
				...
				this.dataService
					.editAliment(this.freezerId, alimFromForm)
					.subscribe(dmlObserver)
			} else {
				this.dataService
					.addAliment(this.freezerId, alimFromForm)
					.subscribe(dmlObserver)
			}
		} else {
