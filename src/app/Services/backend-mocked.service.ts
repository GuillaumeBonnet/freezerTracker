...
export class BackendServiceMocked implements BackendService {
	...
	freezerIdToContentMap = {
		1: [
			...
			new Aliment({
				...
				name: 'camembert',
				category: 'snacks',
				iconicFontName: 'icon-batch1_cheese',
				quantity: 1,
				quantityUnit: 'pieces',
				storedDate: new Date('2019-07-08'),
				expirationDate: new Date('2019-07-10'),
		...
		2: [
			new Aliment({
				...
				iconicFontName: 'icon-batch1_milk-1',
				quantity: 1,
				quantityUnit: 'pieces',
				storedDate: new Date('2019-07-08'),
				expirationDate: new Date('2019-07-10'),
		...
		3: [
			new Aliment({
				...
				name: 'beer',
				category: 'boissons',
				iconicFontName: 'icon-batch1_water-1',
				quantity: 6,
	...
	getUserInfo(): Observable<UserInfo> {
		...
		if (!this.isLoggedIn) {
			return throwError(new Error('mocked getUserInfo error 500.')).pipe(
				catchError((e) =>
					timer(this.fakeDelay).pipe(mergeMap((t) => throwError(e)))
				)
			)
		}
		...
		return of(JSON.parse(JSON.stringify(mockedUserInfo))).pipe(
	...
	login(username: string, password: string): Observable<Object> {
		...
		if (username == 'guest' && password == 'guest-password') {
			...
			return throwError(new Error('User mocked login failed.')).pipe(
				catchError((e) =>
					timer(1000).pipe(mergeMap((t) => throwError(e)))
				)
			)
		}
	}
	...
	getFreezers(): Observable<Freezer[]> {
		...
		if (this.isLoggedIn) {
			return of(JSON.parse(JSON.stringify(this.freezers))).pipe(
	...
	saveFreezer(name: string): Observable<Freezer> {
		...
		if (this.isLoggedIn) {
			let newFreezer: Freezer = new Freezer({
				name: name,
				id: this.freezers.length + 1,
			})
			this.freezers.push(newFreezer)
			return of(JSON.parse(JSON.stringify(newFreezer))).pipe(
				delay(this.fakeDelay)
			)
		} else {
	...
	deleteFreezer(freezerToDelete: Freezer): Observable<Object> {
		...
		if (this.isLoggedIn) {
			if (
				this.freezers.slice(
					this.freezers.findIndex(
						(freezer: Freezer) => freezer.id == freezerToDelete.id
					)
				)
			) {
				return of(null).pipe(delay(this.fakeDelay))
				...
				return throwError(
					new Error('Freezer could not be deleted')
				).pipe(
					catchError((e) =>
						timer(this.fakeDelay).pipe(
							mergeMap((t) => throwError(e))
						)
					)
				)
			}
	...
	updateFreezer(freezerToUpdate: Freezer): Observable<Freezer> {
		...
		if (this.isLoggedIn) {
			let updateFreezerIndex: number = this.freezers.findIndex(
				(freezer: Freezer) => freezer.id == freezerToUpdate.id
			)
			if (updateFreezerIndex != null && updateFreezerIndex != undefined) {
				this.freezers[updateFreezerIndex] = freezerToUpdate
				return of(JSON.parse(JSON.stringify(freezerToUpdate))).pipe(
					delay(this.fakeDelay)
				)
			} else {
	...
	getAliments(freezerId: number): Observable<Aliment[]> {
		...
		if (this.isLoggedIn) {
			...
			if (content) {
				return of(JSON.parse(JSON.stringify(content))).pipe(
					delay(this.fakeDelay)
				)
			} else {
	...
	saveAliment(
		...
		if (this.isLoggedIn) {
			...
			return of(JSON.parse(JSON.stringify(alimentToSave))).pipe(
				delay(this.fakeDelay)
			)
		} else {
	...
	updateAliment(
		...
		if (this.isLoggedIn) {
			...
			if (content) {
				alimentInMap = content.find(
					(aliment) => aliment.id == alimentToUpdate.id
				)
			}
	...
	deleteAliment(
		...
		if (this.isLoggedIn) {
			...
			if (content) {
				indexOfalimentInMap = content.findIndex(
					(aliment) => aliment.id == alimentToDelete.id
				)
			}

			if (
				typeof indexOfalimentInMap === 'number' &&
				indexOfalimentInMap != -1
			) {
