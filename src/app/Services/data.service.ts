...
export class DataService {
	...
	login(username: string, password: string): Observable<Object> {
		...
		return this.backendService
			...
			.pipe(this.shutDownSpinner())
	}

	logout(): Observable<Object> {
		this.spinnerEvent.emit(true)
		return this.backendService.logout().pipe(this.shutDownSpinner())
	}
	...
	getFreezers(): Observable<Freezer[]> {
		if (this.listFreezers) {
			...
			this.spinnerEvent.emit(true)
			return this.backendService.getFreezers().pipe(
				this.shutDownSpinner(),
	...
	editFreezer(freezerWithChanges: Freezer): Observable<Freezer> {
		...
		return this.backendService.updateFreezer(freezerWithChanges).pipe(
			...
			tap({
				next: (result) => {
					Object.assign(
						this.listFreezers.find(
							(freezer) => freezer.id == freezerWithChanges.id
						),
						freezerWithChanges
					)
				},
			...
			switchMap((freezers: Freezer[]) => {
				...
				return this.backendService.deleteFreezer(freezerToDelete).pipe(
					...
					tap(() => {
						freezers.splice(
							freezers.findIndex(
								(elem) => elem.id == freezerToDelete.id
							),
							1
						)
					})
	...
	addAliment(freezerId: number, alimentToAdd: Aliment): Observable<Aliment> {
		return this.getFreezerContent(freezerId).pipe(
			switchMap((freezerContent: Aliment[]) => {
				this.spinnerEvent.emit(true)
				return this.backendService
					.saveAliment(freezerId, alimentToAdd)
					.pipe(
						this.shutDownSpinner(),
	...
	editAliment(
		freezerId: number,
		alimentWithUpdates: Aliment
	): Observable<Aliment> {
		return this.getFreezerContent(freezerId).pipe(
			switchMap((freezerContent: Aliment[]) => {
				this.spinnerEvent.emit(true)
				return this.backendService
					.updateAliment(freezerId, alimentWithUpdates)
					.pipe(
						this.shutDownSpinner(),
						tap((alimentWithUpdates: Aliment) => {
							Object.assign(
								freezerContent.find(
									(alim) => alim.id == alimentWithUpdates.id
								),
								alimentWithUpdates
							)
						})
	...
	deleteAliment(
		...
		return this.getFreezerContent(freezerId).pipe(
			switchMap((freezerContent: Aliment[]) => {
				this.spinnerEvent.emit(true)
				return this.backendService
					.deleteAliment(freezerId, alimentToDelete)
					.pipe(
						this.shutDownSpinner(),
						tap((alimentWithUpdates: Aliment) => {
							freezerContent.splice(
								freezerContent.findIndex(
									(elem) => elem.id == alimentToDelete.id
								),
								1
							)
						})
