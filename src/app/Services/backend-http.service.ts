...
export class BackendServiceHttp implements BackendService {
	...
	getUserInfo(): Observable<UserInfo> {
		return this.http
			.get(`${this.apiRoot}/users/info`)
			.pipe(map((jsonBody: UserInfo) => jsonBody))
	}
	...
	register(registrationInfo: {
		...
		return this.http.post(
			`${this.apiRoot}/users/registration`,
			registrationInfo
		)
	}
	...
	getFreezers(): Observable<Freezer[]> {
		return this.http
			.get(`${this.apiRoot}/freezers`, this.options)
			.pipe(map((jsonBody: Freezer[]) => jsonBody))
	}

	saveFreezer(name: string): Observable<Freezer> {
		return this.http
			.post(
				`${this.apiRoot}/freezers`,
				new Freezer({ name: name }),
				this.options
			)
			.pipe(map((jsonBody: Freezer) => jsonBody))
	}
	...
	deleteFreezer(freezerToDelete: Freezer): Observable<Object> {
		return this.http.delete(
			`${this.apiRoot}/freezers/${freezerToDelete.id}`,
			this.options
		)
	}
	...
	updateFreezer(freezerToUpdate: Freezer): Observable<Freezer> {
		return this.http
			.put(
				`${this.apiRoot}/freezers/${freezerToUpdate.id}`,
				freezerToUpdate,
				this.options
			)
			.pipe(map((jsonBody: Freezer) => jsonBody))
	...
	getAliments(freezerId: number): Observable<Aliment[]> {
		return this.http
			.get(`${this.apiRoot}/freezers/${freezerId}/aliments`, this.options)
			.pipe(map((jsonBody: Aliment[]) => jsonBody))
	}
	...
	saveAliment(
		...
		return this.http
			.post(
				`${this.apiRoot}/freezers/${freezerId}/aliments`,
				alimentToSave,
				this.options
			)
			.pipe(map((jsonBody: Aliment) => jsonBody))
	...
	updateAliment(
		...
		return this.http
			.put(
				`${this.apiRoot}/freezers/${freezerId}/aliments/${alimentToUpdate.id}`,
				alimentToUpdate,
				this.options
			)
			.pipe(map((jsonBody: Aliment) => jsonBody))
	...
	deleteAliment(
		...
		return this.http.delete(
			`${this.apiRoot}/freezers/${freezerId}/aliments/${alimentToDelete.id}`,
			this.options
		)
	}
}
