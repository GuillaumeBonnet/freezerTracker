...
@Component({
	template: `
		<app-aliment
			[aliment]="aliment" [i]="i" [isSelected]="isSelected" (posAlimentSelected)="handledAlimentSelected($event)">
		</app-aliment>`
})
...
describe('AlimentComponent', () => {
	...
	it('should change when is selected', () => {
		...
		expect(h1.className).toBe('aliment-card__title aliment-card__title--selected');
