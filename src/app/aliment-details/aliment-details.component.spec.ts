...
@Component({
	template: `
		<app-aliment-details
			[aliment]="aliment" [freezerId]="freezerId" (posAlimentSelected)="posAlimentSelected(alimentIndex)">
		</app-aliment-details>`
})
...
describe('AlimentDetailsComponent', () => {
	...
	it('should display the freezer fields', () => {
		let fields:NodeListOf<HTMLElement> = component.querySelectorAll('.line');
		...
		expect(expirationDateText).toContain(datePipe.transform(testHost.aliment.expirationDate, 'longDate'));

		let storedDateText:string = fields.item(3).innerText;
		expect(storedDateText).toContain("Stored date");
		expect(storedDateText).toContain(datePipe.transform(testHost.aliment.storedDate, 'longDate'));

		let quantityText:string = fields.item(4).innerText;
		expect(quantityText).toContain("Quantity");
		expect(quantityText).toContain(`${testHost.aliment.quantity} ${testHost.aliment.quantityUnit}`);
	});
	...
	it('should display the right svg', () => {
		...
		expect(useChildNode.attributes.getNamedItem('xlink:href').value).toBe('#' + testHost.aliment.iconicFontName);
	});

	it('should navigate to edit', async(() => {
		let editButton:HTMLElement = component.querySelector('button#aliment-details-edit');
		editButton.click();
		fixture.whenStable()
		.then(() => { // wait for async animation setTimeOut
			expect(routerSpy.navigate).toHaveBeenCalledWith(['edit-aliment', testHost.aliment.id], {relativeTo: stubedRoute});
		})
		.catch((error) => {
	...
	it('should do the deleta action', () => {
		let deleteButton:HTMLElement = component.querySelector('#aliment-details-delete');
		deleteButton.click();
		expect(stubedMatDialog.open).toHaveBeenCalledWith(PopUpDeleteComponent, {
			width: '200px',
			panelClass: 'gs-popup',
			data: {freezerId: testHost.freezerId, alimentId: testHost.aliment.id}
		});
	});
});
