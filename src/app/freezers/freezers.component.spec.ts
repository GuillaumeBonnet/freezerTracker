...
@Component({
	template: ` <app-freezers> </app-freezers>`,
})
...
describe('FreezersComponent', () => {
	...
	let fakeFreezers: Freezer[] = [
		new Freezer({ name: 'name' }),
		new Freezer({ name: 'name2' }),
	]
	let routerSpy
	let dataServiceSpy
	let matDialogSpy
	...
	beforeEach(async(() => {
		...
		TestBed.configureTestingModule({
			declarations: [FreezersComponent, TestHostComponent],
			providers: [
	...
	it('should display the "new freezer" card and tests its functionnality', () => {
		...
		let newFreezerCard: HTMLElement =
			component.querySelector('li.aliment-card')
		...
		fixture.detectChanges()
		expect(dataServiceSpy.addFreezer.calls.first().args[0]).toBe(
			'new freezer name'
		)
	})
